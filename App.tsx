...
> = {
  uk: {
    title: 'DayFlow',
    subtitle: 'Щоденник задач і звітів',
    menuTitle: 'Розділи',
    ...
    taskPresets: 'Шаблони задач',
    taskPresetPlaceholder: 'Новий шаблон задачі',
    addPreset: 'Додати шаблон',
    noPresets: 'Поки що немає шаблонів.',
    editDoneComment: 'Коментар',
    commentTitle: 'Коментар до завершеної задачі',
    commentSave: 'Зберегти',
    commentCancel: 'Скасувати',
    searchPlaceholder: 'Пошук за назвою, коментарем або тегом',
    ...
    noNextTask: 'Немає наступних завдань.',
    ...
    noTasksForDay: 'Ще немає завдань на цей день.',
    allTasks: 'Усі завдання',
    totalTasks: 'Всього',
    pendingTasks: 'У процесі',
    doneTasks: 'Завершено',
    productiveTime: 'Продуктивний час',
    idleTime: 'Простій',
    topTasks: 'Топ-3 завдання за часом',
    delete: 'Видалити',
    error: 'Помилка',
    loadError: 'Не вдалося завантажити збережені записи.',
    saveError: 'Не вдалося зберегти зміни.',
    validationTitle: 'Перевір поля',
    validationMessage: 'Дата, час і назва завдання обовʼязкові.',
    noDataTitle: 'Нема даних',
    noDataMessage: 'Додай хоча б одне завдання перед експортом.',
    done: 'Готово',
...
export default function App() {
  ...
  const c = {
    appBg: isColorful ? '#090F1D' : isDark ? '#0b1324' : '#edf2ff',
    cardBg: isColorful ? '#0f172a' : isDark ? '#111a2e' : '#ffffff',
    cardBorder: isColorful ? '#243244' : isDark ? '#2b3a57' : '#dde6fb',
    textPrimary: isColorful ? '#E5E7EB' : isDark ? '#e5ecff' : '#1b1b1f',
    textSecondary: isColorful ? '#9CA3AF' : isDark ? '#aab9da' : '#606781',
    tabBg: isColorful ? '#0f172a' : isDark ? '#101a2d' : '#ffffff',
    tabBorder: isColorful ? '#2c3a52' : isDark ? '#2c3a54' : '#d8e3fa',
    tabActiveBg: isColorful ? '#312e81' : isDark ? '#25406b' : '#eaf1ff',
          ...
          <View style={styles.pickerRow}>
            <View style={[styles.dateWheelContainer, styles.halfWheel]}>
              <FlatList
                ...
                onScrollEndDrag={(event) => {
                  applyDateMagnetFromOffset(event.nativeEvent.contentOffset.y);
                  scheduleDateMagnet();
                }}
            ...
            <View style={[styles.timeWheelContainer, styles.halfWheel]}>
              <FlatList
                ...
                onScrollEndDrag={(event) => {
                  applyTimeMagnetFromOffset(event.nativeEvent.contentOffset.y);
                  scheduleTimeMagnet();
                }}
...
const styles = StyleSheet.create({
  ...
  scrollContainer: {
    padding: 18,
    paddingTop: Platform.OS === 'web' ? 2 : 18,
    paddingBottom: 98,
  },
  title: {
    fontSize: 28,
